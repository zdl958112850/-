...
//      alert("您还未安装过插件，双击开发包目录里的WebComponents.exe安装！");   
    ...
    // 初始化插件参数及插入插件   
    WebVideoCtrl.I_InitPlugin(800, 580, {   
        iWndowType: 2,   
...
//      alert("检测到新的插件版本，双击开发包目录里的WebComponents.exe升级！");   
        alert("检测到新的插件版本，请下载WebComponents.exe升级！");   
        upload();   
        return;   
    }   
...
function upload(){   
    window.location.href='../codebase/WebComponents.exe';   
}   
    ...
    var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {   
        success: function (xmlDoc) {   
            ...
            setTimeout(function () {   
                $("#ip").val(szIP);   
                getChannelInfo();   
                RealPlayAll();//登陆成功后开始全部预览   
            }, 800);   
               
            changeWndNum(2);//初始化显示画面分割个数   
   
        },   
        error: function () {   
            alert("视频接口，连接失败！请检查网络或设置！");   
...
//  alert($("#ip").val());   
    ...
    var str;   
    $(function(){   
          str = $("#channels option").map(function(){return $(this).val();}).get().join(", ");   
    });   
...
//  var iRet = WebVideoCtrl.I_StartRealPlay('192.168.1.65', {   
//      iWndIndex:0,   
//      iStreamType: 1,   
//      iChannelID: 1,   
//      bZeroChannel: false   
...
function StopPlayAll() {   
    ...
    for (i=0;i<strs.length ;i++ )   
    {   
        var iRet = WebVideoCtrl.I_Stop(strs[i]-1);   
    }   
...
function clickCapturePic() {   
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),   
        szInfo = "";   
alert(WebVideoCtrl.I_GetLocalCfg().toString);   
    if (oWndInfo != null) {   
